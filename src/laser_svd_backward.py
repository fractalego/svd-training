...
if __name__ == "__main__":
    ...
    for layer_index in reversed(range(len(model.base_model.layers))):
        for mlp_name in mlp_names:
            ...
            try:
                exec(
                    f"model.model.layers[layer_index].{mlp_name} = LaserLinear(weight, rank_fraction)"
                )
            except AssertionError as e:
                print(
                    f"Layer {layer_index} on {mlp_name} with Q={rank_fraction} is rejected.\nAssertionError: {e}"
                )
            ...
            print(
                f"Layer {layer_index} on {mlp_name} with Q={rank_fraction} is accepted"
            )
            sys.stdout.flush()

        torch.save(
            model.state_dict(),
            f"mistral-7b-instruct-laser-distil-backward-{rank_fraction}",
        )
